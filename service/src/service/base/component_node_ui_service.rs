...
impl ComponentNodeUiMutation {
    pub async fn create(
        db: &DbConn,
        component_node_ui_model: component_node_ui::Model,
    ) -> Result<component_node_ui::Model, TcdtServiceError> {
        let mut component_node_ui_active_model = component_node_ui::convert_model_to_active_model(component_node_ui_model);
        let id = generate_id();
        component_node_ui_active_model.id_component_node_ui = Set(id.clone());
        let _ = component_node_ui::Entity::insert(component_node_ui_active_model).exec(db)
            .await.map_err(|err| {
            TcdtServiceError::build_internal_msg_error(
                "ComponentNodeUi insert failed",
                err,
            )
        })?;

        let component_node_ui_save = component_node_ui::Entity::find_by_id(id).one(db)
            .await.map_err(|err| {
            TcdtServiceError::build_internal_msg_error(
                "ComponentNodeUi insert after find failed",
                err,
            )
        })?
            .ok_or(TcdtServiceError::build_internal_msg("ComponentNodeUi insert after cannot find entity"))?;
    ...
    pub async fn update_by_id(
        ...
        let id = component_node_ui_model.id_component_node_ui.clone();

        let component_node_ui_persist_model: component_node_ui::ActiveModel = component_node_ui::Entity::find_by_id(&id)
            .one(db)
            .await
            .map_err(|err| {
                TcdtServiceError::build_internal_msg_error(
                    "ComponentNodeUi update before find_by_id failed",
                    err,
                )
            })?
            .ok_or(TcdtServiceError::build_internal_msg(&format!("ComponentNodeUi update before cannot find entity [{}].", stringify!(#entity_name_ident))))?
            .into_active_model();

        let mut component_node_ui_active_model = component_node_ui::convert_model_to_active_model(component_node_ui_model);

        let component_node_ui_save = component_node_ui_active_model
            .update(db)
            .await.map_err(|err| {
            TcdtServiceError::build_internal_msg_error(
                " ComponentNodeUi update failed",
                err,
            )
        })?;
    ...
    pub async fn delete(
        ...
        let delete_result = component_node_ui::Entity::delete(component_node_ui_model.into_active_model())
            .exec(db)
            .await
            .map_err(|err| {
    ...
    pub async fn batch_delete(
        db: &DbConn,
        component_node_ui_model_list: Vec<component_node_ui::Model>,
    ) -> Result<DeleteResult, TcdtServiceError> {
        let id_list = component_node_ui_model_list.iter().map(|component_node_ui_model| {
            component_node_ui_model.id_component_node_ui.clone()
        }).collect::<Vec<String>>();
        let delete_result = component_node_ui::Entity::delete_many()
            .filter(Expr::col(component_node_ui::Column::IdComponentNodeUi).is_in(id_list))
            .exec(db)
            .await
            .map_err(|err| {
                log::error!("ComponentNodeUi batch_delete failed");
                TcdtServiceError::build_internal_msg_error("ComponentNodeUi batch_delete failed", err)
            })?;
        Ok(delete_result)
    }
...
impl ComponentNodeUiQuery {
    ...
    pub async fn find_by_ids(
        ...
        let aq_condition = AqCondition::build_in_condition("idComponentNodeUi", ids
            .iter()
