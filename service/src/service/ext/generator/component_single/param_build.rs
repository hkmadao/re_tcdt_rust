use crate::dto::{
    po::ext::generate::component_single::{
...
pub(crate) async fn build(
    ...
    let child_entity_info_list = entity_vo_list
        .clone()
        .into_iter()
        .filter(|entity_vo| !entity_vo.fg_main)
        .collect::<Vec<_>>();

    let all_up_out_ref_entity_info_list = entity_vo_list
        .clone()
        .into_iter()
        .flat_map(|entity_info| entity_info.up_entity_info_list)
        .collect::<Vec<_>>();
    let all_down_entity_info_list = entity_vo_list
        .clone()
        .into_iter()
...
fn build_desc_info(
    ...
    let ref_attr_info_list = all_attr_info_list
        ...
        .filter(|attr| {
            ...
            //     attr.attribute_name.clone().unwrap_or_default(),
            //     attr.domain_type_code.clone().unwrap_or_default()
            // );
    ...
    let result_entity_info = EntityInfoPO {
        ...
        comp_info: entity_info.comp_info.clone(),
