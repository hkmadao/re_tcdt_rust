...
pub async fn add(
    ...
    let node_ui_model = NodeUiPO::convert_po_to_model(form);

    let node_ui_save = NodeUiMutation::create(conn, node_ui_model)
        .await
        .map_err(|e| {
            log::error!("{:?}", e);
...
pub async fn update(
    ...
    let node_ui_save = NodeUiMutation::update_by_id(conn, node_ui_model)
        .await
        .map_err(|e| {
            log::error!("{:?}", e);
            error::ErrorInternalServerError("internal server error")
...
pub async fn remove(
    ...
    let delete_result = NodeUiMutation::delete(conn, node_ui_model)
        .await
        .map_err(|e| {
            log::error!("{:?}", e);
            error::ErrorInternalServerError("internal server error")
...
#[post("/nodeUi/batchRemove")]
pub async fn batch_remove(
    data: web::Data<AppState>,
    node_ui_form: web::Json<Vec<NodeUiPO>>,
) -> Result<HttpResponse, Error> {
    let conn = &data.conn;
    let po_list = node_ui_form.into_inner();

    let mut model_list:Vec<node_ui::Model>  = vec![];
    for po in po_list {
        model_list.push(NodeUiPO::convert_po_to_model(po));
    }
    
    let delete_result = NodeUiMutation::batch_delete(conn, model_list)
        .await
        .map_err(|e| {
            log::error!("{:?}", e);
            error::ErrorInternalServerError("internal server error")
