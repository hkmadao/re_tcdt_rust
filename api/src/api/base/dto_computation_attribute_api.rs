...
pub async fn add(
    ...
    let dto_computation_attribute_model = DtoComputationAttributePO::convert_po_to_model(form);

    let dto_computation_attribute_save = DtoComputationAttributeMutation::create(conn, dto_computation_attribute_model)
        .await
        .map_err(|e| {
            log::error!("{:?}", e);
...
pub async fn update(
    ...
    let dto_computation_attribute_save = DtoComputationAttributeMutation::update_by_id(conn, dto_computation_attribute_model)
        .await
        .map_err(|e| {
            log::error!("{:?}", e);
            error::ErrorInternalServerError("internal server error")
...
pub async fn remove(
    ...
    let delete_result = DtoComputationAttributeMutation::delete(conn, dto_computation_attribute_model)
        .await
        .map_err(|e| {
            log::error!("{:?}", e);
            error::ErrorInternalServerError("internal server error")
...
#[post("/dtoComputationAttribute/batchRemove")]
pub async fn batch_remove(
    data: web::Data<AppState>,
    dto_computation_attribute_form: web::Json<Vec<DtoComputationAttributePO>>,
) -> Result<HttpResponse, Error> {
    let conn = &data.conn;
    let po_list = dto_computation_attribute_form.into_inner();

    let mut model_list:Vec<dto_computation_attribute::Model>  = vec![];
    for po in po_list {
        model_list.push(DtoComputationAttributePO::convert_po_to_model(po));
    }
    
    let delete_result = DtoComputationAttributeMutation::batch_delete(conn, model_list)
        .await
        .map_err(|e| {
            log::error!("{:?}", e);
            error::ErrorInternalServerError("internal server error")
