...
pub async fn add(
    ...
    let role_menu_model = RoleMenuPO::convert_po_to_model(form);

    let role_menu_save = RoleMenuMutation::create(conn, role_menu_model)
        .await
        .map_err(|e| {
            log::error!("{:?}", e);
...
pub async fn update(
    ...
    let role_menu_save = RoleMenuMutation::update_by_id(conn, role_menu_model)
        .await
        .map_err(|e| {
            log::error!("{:?}", e);
            error::ErrorInternalServerError("internal server error")
...
pub async fn remove(
    ...
    let delete_result = RoleMenuMutation::delete(conn, role_menu_model)
        .await
        .map_err(|e| {
            log::error!("{:?}", e);
            error::ErrorInternalServerError("internal server error")
...
pub async fn batch_remove(
    ...
    let mut model_list:Vec<role_menu::Model>  = vec![];
    for po in po_list {
        model_list.push(RoleMenuPO::convert_po_to_model(po));
    }
    
    let delete_result = RoleMenuMutation::batch_delete(conn, model_list)
        .await
        .map_err(|e| {
            log::error!("{:?}", e);
            error::ErrorInternalServerError("internal server error")
