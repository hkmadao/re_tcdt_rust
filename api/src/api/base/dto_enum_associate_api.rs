...
pub async fn add(
    ...
    let dto_enum_associate_model = DtoEnumAssociatePO::convert_po_to_model(form);

    let dto_enum_associate_save = DtoEnumAssociateMutation::create(conn, dto_enum_associate_model)
        .await
        .map_err(|e| {
            log::error!("{:?}", e);
...
pub async fn update(
    ...
    let dto_enum_associate_save = DtoEnumAssociateMutation::update_by_id(conn, dto_enum_associate_model)
        .await
        .map_err(|e| {
            log::error!("{:?}", e);
            error::ErrorInternalServerError("internal server error")
...
pub async fn remove(
    ...
    let delete_result = DtoEnumAssociateMutation::delete(conn, dto_enum_associate_model)
        .await
        .map_err(|e| {
            log::error!("{:?}", e);
            error::ErrorInternalServerError("internal server error")
...
#[post("/dtoEnumAssociate/batchRemove")]
pub async fn batch_remove(
    data: web::Data<AppState>,
    dto_enum_associate_form: web::Json<Vec<DtoEnumAssociatePO>>,
) -> Result<HttpResponse, Error> {
    let conn = &data.conn;
    let po_list = dto_enum_associate_form.into_inner();

    let mut model_list:Vec<dto_enum_associate::Model>  = vec![];
    for po in po_list {
        model_list.push(DtoEnumAssociatePO::convert_po_to_model(po));
    }
    
    let delete_result = DtoEnumAssociateMutation::batch_delete(conn, model_list)
        .await
        .map_err(|e| {
            log::error!("{:?}", e);
            error::ErrorInternalServerError("internal server error")
