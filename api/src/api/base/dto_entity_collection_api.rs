...
pub async fn add(
    ...
    let dto_entity_collection_model = DtoEntityCollectionPO::convert_po_to_model(form);

    let dto_entity_collection_save = DtoEntityCollectionMutation::create(conn, dto_entity_collection_model)
        .await
        .map_err(|e| {
            log::error!("{:?}", e);
...
pub async fn update(
    ...
    let dto_entity_collection_save = DtoEntityCollectionMutation::update_by_id(conn, dto_entity_collection_model)
        .await
        .map_err(|e| {
            log::error!("{:?}", e);
            error::ErrorInternalServerError("internal server error")
...
pub async fn remove(
    ...
    let delete_result = DtoEntityCollectionMutation::delete(conn, dto_entity_collection_model)
        .await
        .map_err(|e| {
            log::error!("{:?}", e);
            error::ErrorInternalServerError("internal server error")
...
#[post("/dtoEntityCollection/batchRemove")]
pub async fn batch_remove(
    data: web::Data<AppState>,
    dto_entity_collection_form: web::Json<Vec<DtoEntityCollectionPO>>,
) -> Result<HttpResponse, Error> {
    let conn = &data.conn;
    let po_list = dto_entity_collection_form.into_inner();

    let mut model_list:Vec<dto_entity_collection::Model>  = vec![];
    for po in po_list {
        model_list.push(DtoEntityCollectionPO::convert_po_to_model(po));
    }
    
    let delete_result = DtoEntityCollectionMutation::batch_delete(conn, model_list)
        .await
        .map_err(|e| {
            log::error!("{:?}", e);
            error::ErrorInternalServerError("internal server error")
