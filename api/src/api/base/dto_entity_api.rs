...
pub async fn add(
    ...
    let dto_entity_model = DtoEntityPO::convert_po_to_model(form);

    let dto_entity_save = DtoEntityMutation::create(conn, dto_entity_model)
        .await
        .map_err(|e| {
            log::error!("{:?}", e);
...
pub async fn update(
    ...
    let dto_entity_save = DtoEntityMutation::update_by_id(conn, dto_entity_model)
        .await
        .map_err(|e| {
            log::error!("{:?}", e);
            error::ErrorInternalServerError("internal server error")
...
pub async fn remove(
    ...
    let delete_result = DtoEntityMutation::delete(conn, dto_entity_model)
        .await
        .map_err(|e| {
            log::error!("{:?}", e);
            error::ErrorInternalServerError("internal server error")
...
#[post("/dtoEntity/batchRemove")]
pub async fn batch_remove(
    data: web::Data<AppState>,
    dto_entity_form: web::Json<Vec<DtoEntityPO>>,
) -> Result<HttpResponse, Error> {
    let conn = &data.conn;
    let po_list = dto_entity_form.into_inner();

    let mut model_list:Vec<dto_entity::Model>  = vec![];
    for po in po_list {
        model_list.push(DtoEntityPO::convert_po_to_model(po));
    }
    
    let delete_result = DtoEntityMutation::batch_delete(conn, model_list)
        .await
        .map_err(|e| {
            log::error!("{:?}", e);
            error::ErrorInternalServerError("internal server error")
